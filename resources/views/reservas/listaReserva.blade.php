...
@section('content')
    ...
    {{-- recorrer los eventos de reservas de citas medicas --}}
    <x-adminlte-datatable id="table11" :heads="$heads" head-theme="light" :config="$config" striped hoverable bordered
        compressed>
        ...
        @foreach ($eventos as $evento)
            <tr>
                ...
                {{-- DESTROY data  --}}
                <td>
                    <form style="display: inline" action="{{ route('home.destroy', $evento->id) }}" method="POST"
                        class="formEliminar">
                        @csrf
                        @method('DELETE')
                        {!! $btnDelete !!}
