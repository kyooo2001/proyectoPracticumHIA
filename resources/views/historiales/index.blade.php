...
@section('content')
    ...
    {{-- Setup data for datatables for create historiales --}}
    @hasanyrole('doctor')
        <div class="card">
            <div class="card-body">
                <div class="text-right">
                    <a href="{{ url('historiales/create') }}" class="btn btn-primary btn-lg active" data-mdb-ripple-init
                ...
                @if (Auth::check() && Auth::user()->doctor) {{-- Verifica que el usuario está autenticado y tiene relación con doctor --}}
                    @foreach ($historiales as $historiale)
                        @if ($historiale->doctor_id == Auth::user()->doctor->id)
                            {{-- Valida el historial con id del doctor --}}
                            <tr>
                                <td>{{ $contador++ }}</td>
                                <td>
                                ...
                                <td><a href= "{{ route('historiales.edit', $historiale) }}"
                                        class= "btn btn-xs btn-default text-primary mx-1 shadow" title="Edit">
                                        <i class="fa fa-lg fa-fw fa-pen"></i>
                                    </a>
                                    {{-- DESTROY data  --}}

                                    <form style="display: inline" action="{{ route('historiales.destroy', $historiale) }}"
                                    ...
                                    <a href= "{{ route('historiales.show', $historiale) }}"
                                        class="btn btn-xs btn-default text-teal mx-1 shadow" title="Details">
                                        <i class="fa fa-lg fa-fw fa-eye"></i>
                                    </a>
                                    {{-- Botón de impresión que redirige a reporteh --}}
                                    <a href="{{ route('historiales.reporteh', $historiale->id) }}"
                                        class="btn btn-xs btn-warning text-dark mx-1 shadow" title="Print Report">
